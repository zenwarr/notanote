...
import { StorageConfig, StorageProviderManager } from "../storage/storage-provider";
import { StorageConfigView } from "./storage-config-view";
...
async function initialize(config?: StorageConfig): Promise<boolean> {
  ...
  const localStorage = localStorageProvider.storageFactory(config.local.options);
