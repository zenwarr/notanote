...
const PluginConfigEditor = observer((props: { doc: Document }) => {
  ...
  async function update(pluginName: string) {
    const wsId = Workspace.instance.remoteStorageName;
...
function CloneDialog(props: { open: boolean, onClose: () => void, doc: Document }) {
  async function clone(params: { url: string | undefined, name: string | undefined }) {
    ...
    try {
      await Backend.get(PluginBackend).clone(Workspace.instance.remoteStorageName, params.name, params.url);
      props.onClose();
    } catch (error: any) {
...
class PluginConfigEditorState implements DocumentEditorStateAdapter {
  constructor(doc: Document) {
    ...
    try {
      this.data = JSON.parse(doc.getLastSavedData().toString());
    } catch (err: any) {
      this.data = {};
    }
  }
